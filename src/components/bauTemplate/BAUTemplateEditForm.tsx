...
import { CreateBAUTemplateFormData, BAUTemplateWithDetails } from "@/types/bauTemplate";
...
interface BAUTemplateEditFormProps {
  template: BAUTemplateWithDetails | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (templateId: string, data: CreateBAUTemplateFormData) => Promise<boolean>;
}

export function BAUTemplateEditForm({ template, open, onOpenChange, onSubmit }: BAUTemplateEditFormProps) {
  ...
  useEffect(() => {
    if (template && open) {
      const assignedMembers = template.template_assignments?.map(assignment => assignment.user_id) || [];
      const kpis = template.template_kpis?.map(kpi => ({
        name: kpi.name,
        unit: kpi.unit,
        operator: kpi.operator,
        target_value: kpi.target_value,
        source: kpi.source || "",
      })) || [{ name: "", unit: "Percentage", operator: "GreaterThanEqual", target_value: 0, source: "" }];
  ...
  const handleSubmit = async (data: CreateBAUTemplateFormData) => {
    ...
    const success = await onSubmit(template.id, data);
    if (success) {
      onOpenChange(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit BAU Template</DialogTitle>
        </DialogHeader>
